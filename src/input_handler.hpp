...
public:
	void update();
	void sendMouseWheelEvent(const MouseWheelType type);
	void sendMouseDownEvent(const MouseButtonType type);
	void sendMouseUpEvent(const MouseButtonType type);
	void sendKeyboardDownEvent(const KeyboardKeyType type);
	void sendKeyboardUpEvent(const KeyboardKeyType type);

	const bool isMouseWheelScrolled(const MouseWheelType type) const;
	const bool isMouseButtonHeld(const MouseButtonType type) const;
	const bool isMouseButtonPressed(const MouseButtonType type) const;
	const bool isMouseButtonToggled(const MouseButtonType type) const;
	const bool isKeyboardKeyHeld(const KeyboardKeyType type) const;
	const bool isKeyboardKeyPressed(const KeyboardKeyType type) const;
	const bool isKeyboardKeyToggled(const KeyboardKeyType type) const;
